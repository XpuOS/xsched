...
typedef enum CUdevice_attribute_enum {
    ...
    CU_DEVICE_ATTRIBUTE_MAX_PERSISTING_L2_CACHE_SIZE = 106,                 /**< Maximum L2 persisting lines capacity setting in bytes. */
...
typedef enum CUsynchronizationPolicy_enum {
    ...
    typedef enum CUclusterSchedulingPolicy_enum
    {
        ...
        CU_CLUSTER_SCHEDULING_POLICY_LOAD_BALANCING =
            2 /**< allow the hardware to load-balance the blocks in a cluster to the SMs */
    } CUclusterSchedulingPolicy;

    /**
     * Memory Synchronization Domain
     *
     * A kernel can be launched in a specified memory synchronization domain that affects all memory operations issued
     * by that kernel. A memory barrier issued in one domain will only order memory operations in that domain, thus
     * eliminating latency increase from memory barriers ordering unrelated traffic.
     *
     ...
     * graph node / kernel launch. See ::CU_LAUNCH_ATTRIBUTE_MEM_SYNC_DOMAIN, ::cuStreamSetAttribute,
     * ::cuLaunchKernelEx,
     * ::cuGraphKernelNodeSetAttribute.
     *
     * Memory operations done in kernels launched in different domains are considered system-scope distanced. In other
     * words, a GPU scoped memory synchronization is not sufficient for memory order to be observed by kernels in
     * another memory synchronization domain even if they are on the same GPU.
     */
    ...
    typedef enum CUlaunchAttributeID_enum
    {
        ...
        CU_LAUNCH_ATTRIBUTE_CLUSTER_DIMENSION =
            4 /**< Valid for graph nodes, launches. See ::CUlaunchAttributeValue::clusterDim. */
        ,
        CU_LAUNCH_ATTRIBUTE_CLUSTER_SCHEDULING_POLICY_PREFERENCE =
            5 /**< Valid for graph nodes, launches. See ::CUlaunchAttributeValue::clusterSchedulingPolicyPreference. */
        ,
        ...
        CU_LAUNCH_ATTRIBUTE_PREFERRED_CLUSTER_DIMENSION =
            11 /**< Valid for graph nodes, launches. Set
        ...
        CU_LAUNCH_ATTRIBUTE_DEVICE_UPDATABLE_KERNEL_NODE =
            13 /**< Valid for graph nodes, launches. This attribute is graphs-only,
        ...
        CU_LAUNCH_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT =
            14 /**< Valid for launches. On devices where the L1 cache and shared memory use the
        ...
        CU_LAUNCH_ATTRIBUTE_NVLINK_UTIL_CENTRIC_SCHEDULING =
            16 /**< Valid for streams, graph nodes, launches. This attribute is a hint to the CUDA runtime that the
                    launch should attempt to make the kernel maximize its NVLINK utilization.
                    <br>
                    When possible to honor this hint, CUDA will assume each block in the grid launch will carry out an
                  even amount of NVLINK traffic, and make a best-effort attempt to adjust the kernel launch based on
                  that assumption. <br> This attribute is a hint only. CUDA makes no functional or performance
                  guarantee. Its applicability can be affected by many different factors, including driver version (i.e.
                  CUDA doesn't guarantee the performance characteristics will be maintained between driver versions or a
                  driver update could alter or regress previously observed perf characteristics.) It also doesn't
                  guarantee a successful result, i.e. applying the attribute may not improve the performance of either
                  the targeted kernel or the encapsulating application. <br> Valid values for
    ...
    typedef union CUlaunchAttributeValue_union
    {
        ...
        CUaccessPolicyWindow
            accessPolicyWindow; /**< Value of launch attribute ::CU_LAUNCH_ATTRIBUTE_ACCESS_POLICY_WINDOW. */
        int cooperative;        /**< Value of launch attribute ::CU_LAUNCH_ATTRIBUTE_COOPERATIVE. Nonzero indicates a
         ...
         *      - \p triggerAtBlockStart - If this is set to non-0, each block launch will automatically trigger the
         * event.
         */
        struct
        {
         ...
         *     - \p int flags; - Event record flags, see ::cuEventRecordWithFlags. Does not accept
        ...
        unsigned int
            sharedMemCarveout; /**< Value of launch attribute ::CU_LAUNCH_ATTRIBUTE_PREFERRED_SHARED_MEMORY_CARVEOUT. */
 ...
 */
     ...
     * ::cuModuleLoadDataEx
     */
    CUresult CUDAAPI cuLibraryLoadData(CUlibrary*       library,
                                       const void*      code,
     ...
     * ::cuModuleLoadDataEx
     */
    CUresult CUDAAPI cuLibraryLoadFromFile(CUlibrary*       library,
                                           const char*      fileName,
                                           CUjit_option*    jitOptions,
                                           void**           jitOptionsValues,
                                           unsigned int     numJitOptions,
...
CUresult CUDAAPI cuGraphExecChildGraphNodeSetParams(CUgraphExec hGraphExec, CUgraphNode node, CUgraph childGraph);
 ...
 * ::cuGraphExecDestroy
 */
CUresult CUDAAPI cuGraphInstantiate(CUgraphExec *phGraphExec,
            CUgraph      hGraph,
...
CUresult CUDAAPI cuGraphAddEventRecordNode(CUgraphNode *pGraphNode, CUgraph graph, const CUgraphNode *pDependencies, size_t numDependencies, CUevent event);

CUresult CUDAAPI cuGraphEventRecordNodeSetEvent(CUgraphNode hNode, CUevent event);

CUresult CUDAAPI cuGraphEventWaitNodeSetEvent(CUgraphNode node, CUevent event);

CUresult CUDAAPI cuGraphEventRecordNodeGetEvent(CUgraphNode hNode, CUevent* event);

CUresult CUDAAPI cuGraphEventWaitNodeGetEvent(CUgraphNode node, CUevent* event);
...
CUresult CUDAAPI cuGraphAddEventWaitNode(CUgraphNode *pGraphNode, CUgraph graph, const CUgraphNode *pDependencies, size_t numDependencies, CUevent event);

CUresult CUDAAPI cuStreamGetCaptureInfo_v2(CUstream hStream, CUstreamCaptureStatus *pCaptureStatus, cuuint64_t *pId, CUgraph *graph_out, const CUgraphNode **dependencies_out, size_t *numDependencies_out);

CUresult CUDAAPI cuStreamUpdateCaptureDependencies(CUstream stream, CUgraphNode *dependencies, size_t numDependencies, unsigned int flags);
 ...
 * 
 * Using the provided \p hGraph, write to \p path a DOT formatted description of the \p graph.
 * By default this includes the graph topology, node types, node id, kernel names and memcpy direction.
 * \p flags can be specified to write more detailed information about each node type such as parameter values, kernel
 ...
 *      
 */
CUresult CUDAAPI cuGraphDebugDotPrint(CUgraph      hGraph,
                                          const char*  path,
                                          unsigned int flags);
 ...
 * The Id is unique for the life of the program for this instance of CUDA. If green context
 * is supplied as NULL and the current context is set to a green context, the Id of the current
 * green context is returned.
 *
 * \note Note that this function may also return error codes from previous, asynchronous launches.
 *
 ...
 * API dumps all driver logs starting from \p iterator into \p pathToFile provided.
 *
 ...
 *
 * Logs generated by the driver are stored in an internal buffer and can be copied out using this API. This
 * API dumps driver logs from \p iterator into \p buffer up to the size specified in \p *size. The driver
 * will always null terminate the buffer but there will not be a null character between log entries, only
 * a newline \n. The driver will then return the actual number of bytes written in \p *size, excluding the
 * null terminator. If there are no messages to dump, \p *size will be set to 0 and the function will return
 * ::CUDA_SUCCESS. If the provided \p buffer is not large enough to hold any messages, \p *size will be
