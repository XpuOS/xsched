...
typedef struct { unsigned int x, y, z; } x_dim3;
typedef struct CUstream_st *x_cudaStream_t;
typedef enum { x_cudaSuccess = 0 } x_cudaError_t;
...
static void ForceRegister() {
    ...
    CUstream dummy;
    CUresult res = xsched::cuda::XStreamCreate(&dummy, 0);
    if (res == 0) {
        fprintf(stderr, "[XSched-Shim] >>> Registration SUCCESS! <<<\n");
        success = true;
    }
}

// 构造函数：库加载时立即运行
extern "C" __attribute__((constructor)) void shim_init_log() {
    fprintf(stderr, "\n[XSched-Shim] VERSION: RECOVERY-MODE LOADED\n");
    fflush(stderr);
}
...
DEFINE_EXPORT_C_REDIRECT_CALL(XStreamSynchronize, CUresult, cuStreamSynchronize_ptsz, CUstream, hStream);
...
DEFINE_EXPORT_C_REDIRECT_CALL(XMemcpyHtoD_v2, CUresult, cuMemcpyHtoD_v2, CUdeviceptr, dstDevice, const void *, srcHost, size_t, ByteCount);
DEFINE_EXPORT_C_REDIRECT_CALL(XMemcpyDtoH_v2, CUresult, cuMemcpyDtoH_v2, void *, dstHost, CUdeviceptr, srcDevice, size_t, ByteCount);
...
// ============================================================================
// === MANUAL INTERCEPTORS (Driver & Runtime) ===
// ============================================================================

extern "C" __attribute__((visibility("default"))) CUresult cuInit(unsigned int Flags) {
    ForceRegister();
    return Driver::Init(Flags);
}

extern "C" __attribute__((visibility("default"))) CUresult cuLaunchKernel(CUfunction f, unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ, unsigned int sharedMemBytes, CUstream hStream, void **kernelParams, void **extra) {
    return xsched::cuda::XLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams, extra);
}

extern "C" __attribute__((visibility("default"))) CUresult cuLaunchKernel_ptsz(CUfunction f, unsigned int gridDimX, unsigned int gridDimY, unsigned int gridDimZ, unsigned int blockDimX, unsigned int blockDimY, unsigned int blockDimZ, unsigned int sharedMemBytes, CUstream hStream, void **kernelParams, void **extra) {
    return xsched::cuda::XLaunchKernel(f, gridDimX, gridDimY, gridDimZ, blockDimX, blockDimY, blockDimZ, sharedMemBytes, hStream, kernelParams, extra);
}

extern "C" __attribute__((visibility("default"))) CUresult cuLaunchKernelEx(const CUlaunchConfig *config, CUfunction f, void **kernelParams, void **extra) {
    return xsched::cuda::XLaunchKernelEx(config, f, kernelParams, extra);
}

extern "C" __attribute__((visibility("default"))) CUresult cuLaunchKernelEx_ptsz(const CUlaunchConfig *config, CUfunction f, void **kernelParams, void **extra) {
    return xsched::cuda::XLaunchKernelEx(config, f, kernelParams, extra);
}

extern "C" __attribute__((visibility("default"))) x_cudaError_t cudaMalloc(void **ptr, size_t size) {
    static bool first = true;
    if (first) { 
        fprintf(stderr, "\033[1;36m[XSched-Shim] >>> First cudaMalloc intercepted! <<<\033[0m\n"); 
        first = false; 
        ForceRegister(); 
    }
    typedef x_cudaError_t (*pfn)(void**, size_t);
    static pfn real_func = (pfn)dlsym(RTLD_NEXT, "cudaMalloc");
    return real_func ? real_func(ptr, size) : (x_cudaError_t)1;
}

extern "C" __attribute__((visibility("default"))) x_cudaError_t cudaStreamCreate(x_cudaStream_t *pStream) {
    return (x_cudaError_t)xsched::cuda::XStreamCreate((CUstream*)pStream, 0);
}

extern "C" __attribute__((visibility("default"))) x_cudaError_t cudaSetDevice(int device) {
    ForceRegister();
    typedef x_cudaError_t (*pfn)(int);
    static pfn real_func = (pfn)dlsym(RTLD_NEXT, "cudaSetDevice");
    return real_func ? real_func(device) : (x_cudaError_t)1;
}

extern "C" __attribute__((visibility("default"))) x_cudaError_t cudaLaunchKernel(const void *func, x_dim3 gridDim, x_dim3 blockDim, void **args, size_t sharedMem, x_cudaStream_t stream) {
    // Bridge to XLaunchKernelRuntime
    return (x_cudaError_t)xsched::cuda::XLaunchKernelRuntime(
        func,
        gridDim.x, gridDim.y, gridDim.z,
        blockDim.x, blockDim.y, blockDim.z,
        sharedMem, args, (CUstream)stream
    );
}
...
extern "C" __attribute__((visibility("default"))) x_cudaError_t cudaLaunchKernelEx(const void *config, const void *func, void **args) {
    const x_cudaLaunchConfig *cfg = (const x_cudaLaunchConfig *)config;
    return (x_cudaError_t)xsched::cuda::XLaunchKernelRuntime(
        func,
        cfg->gridDim.x, cfg->gridDim.y, cfg->gridDim.z,
        cfg->blockDim.x, cfg->blockDim.y, cfg->blockDim.z,
        cfg->sharedMem, args, (CUstream)cfg->stream
    );
}
...
static const std::unordered_map<std::string, std::map<int, void *>> intercept_funcs = {
    ...
    { "cuLaunchKernel_ptsz"                                 , {{  2000, (void *)cuLaunchKernel_ptsz                                 }}},
    { "cuLaunchKernelEx"                                    , {{  2000, (void *)cuLaunchKernelEx                                    }}},
    { "cuLaunchKernelEx_ptsz"                               , {{  2000, (void *)cuLaunchKernelEx_ptsz                               }}},
};
...
EXPORT_C_FUNC CUresult XGetProcAddress(const char *symbol, void **pfn, int cudaVersion, cuuint64_t flags)
{
    fprintf(stderr, "[XSched-Shim] GetProcAddress Lookup: %s\n", symbol);
    fflush(stderr);
...
EXPORT_C_FUNC CUresult XGetProcAddress_v2(const char *symbol, void **pfn, int cudaVersion, cuuint64_t flags, CUdriverProcAddressQueryResult *symbolStatus)
{
    fprintf(stderr, "[XSched-Shim] Lookup: %s\n", symbol);
    fflush(stderr);
