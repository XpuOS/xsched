...
public:
    ...
    static std::shared_ptr<TensorDesc> Destroy(const aclTensorDesc *desc);
...
public:
    ...
    static bool IsHostAddr(const void *addr);
    static std::shared_ptr<DataBuffer> Create(const aclDataBuffer *buffer, bool deep_copy);
    static std::shared_ptr<DataBuffer> Destroy(const aclDataBuffer *buffer);

private:
    const aclDataBuffer *buffer_ = nullptr;
    void *snapshot_data_ = nullptr;
...
public:
    ...
    static std::shared_ptr<OpAttr> Destroy(const aclopAttr *attr);
...
private:
    ...
    const std::string op_path_;
    const aclopEngineType engine_type_;
    const aclopCompileType compile_flag_;
    std::shared_ptr<OpAttr> attr_ = nullptr;

    std::vector<std::shared_ptr<TensorDesc>> input_descs_;
    std::vector<std::shared_ptr<DataBuffer>> inputs_;
    std::vector<std::shared_ptr<TensorDesc>> output_descs_;
    std::vector<std::shared_ptr<DataBuffer>> outputs_;

    std::vector<const aclTensorDesc *> input_descs_ptrs_;
    std::vector<const aclDataBuffer *> inputs_ptrs_;
    std::vector<const aclTensorDesc *> output_descs_ptrs_;
    std::vector<aclDataBuffer *> outputs_ptrs_;
};
...
ACL_NN_COMMAND(InplaceUniformTensor);
ACL_NN_COMMAND(InplaceRandom);
ACL_NN_COMMAND(InplaceRandomTensor);
ACL_NN_COMMAND(Multinomial);
ACL_NN_COMMAND(MultinomialTensor);
ACL_NN_COMMAND(InplaceNormal);
ACL_NN_COMMAND(InplaceNormalTensor);
ACL_NN_COMMAND(DropoutGenMask);
ACL_NN_COMMAND(DropoutGenMaskV2);
ACL_NN_COMMAND(DropoutGenMaskV2Tensor);
...
// ACL_NN_COMMAND(CalculateMatmulWeightSizeV2); // command without stream
// ACL_NN_COMMAND(CalculateMatmulWeightSize);   // command without stream
ACL_NN_COMMAND(TransMatmulWeight);
// ACL_NN_COMMAND(CalculateConvolutionWeightSize); // command without stream
...
ACL_NN_COMMAND(NpuFormatCast);
// ACL_NN_COMMAND(NpuFormatCastCalculateSizeAndFormat); // command without stream
...
ACL_NN_COMMAND(InplaceFillDiagonal);
...
ACL_COMMAND(AclStftCommand, AclCommand, aclnnStatus, NnOp::Stft, void *, workspace, uint64_t, workspaceSize, aclOpExecutor *, executor);
ACL_NN_COMMAND(Square);
ACL_NN_COMMAND(Sqrt);
...
ACL_NN_COMMAND(SignBitsUnpack);
ACL_NN_COMMAND(SignBitsPack);
ACL_NN_COMMAND(Sign);
ACL_NN_COMMAND(SWhere);
ACL_NN_COMMAND(ExpSegsum);
...
ACL_NN_COMMAND(RightShift);
ACL_COMMAND(AclRfft1DCommand, AclCommand, aclnnStatus, NnOp::Rfft1D, void *, workspace, uint64_t, workspaceSize, aclOpExecutor *, executor);
...
ACL_NN_COMMAND(Qr);
ACL_NN_COMMAND(PrecisionCompareV2);
...
ACL_NN_COMMAND(ExpSegsumBackward);
ACL_NN_COMMAND(Exp);
...
ACL_NN_COMMAND(Cumprod);
ACL_NN_COMMAND(InplaceCumprod);
...
ACL_NN_COMMAND(BitwiseAndTensorOut);
ACL_NN_COMMAND(InplaceBitwiseAndTensorOut);
...
ACL_NN_COMMAND(AddV3);
ACL_NN_COMMAND(InplaceAddV3);
ACL_NN_COMMAND(Add);
ACL_NN_COMMAND(Adds);
...
ACL_NN_COMMAND(Sum);
ACL_NN_COMMAND(Abs);
ACL_NN_COMMAND(SimThreadExponential);
ACL_NN_COMMAND(UnfoldGrad);
ACL_NN_COMMAND(StridedSliceAssignV2);
ACL_NN_COMMAND(TransformBiasRescaleQkv);
ACL_NN_COMMAND(Rfft1D);
ACL_NN_COMMAND(IsInf);
ACL_NN_COMMAND(HansEncode);
ACL_NN_COMMAND(HansDecode);
ACL_NN_COMMAND(AddLora);
ACL_NN_COMMAND(RoiAlignV2Backward);
ACL_NN_COMMAND(RoiAlignV2);
...
ACL_NN_COMMAND(ThreeInterpolateBackward);
...
ACL_NN_COMMAND(RingAttentionUpdateV2);
ACL_NN_COMMAND(RecurrentGatedDeltaRule);
ACL_NN_COMMAND(RainFusionAttention);
ACL_NN_COMMAND(PromptFlashAttentionV3);
ACL_NN_COMMAND(PromptFlashAttentionV2);
ACL_NN_COMMAND(PromptFlashAttention);
ACL_NN_COMMAND(NsaSelectedAttentionInfer);
ACL_NN_COMMAND(NsaSelectedAttentionGrad);
ACL_NN_COMMAND(NsaSelectedAttention);
ACL_NN_COMMAND(NsaCompressWithCache);
ACL_NN_COMMAND(NsaCompressAttentionInfer);
ACL_NN_COMMAND(NsaCompressAttention);
ACL_NN_COMMAND(NsaCompress);
ACL_NN_COMMAND(MlaPrologV3WeightNz);
ACL_NN_COMMAND(MlaPrologV2WeightNz);
ACL_NN_COMMAND(LightningIndexerGrad);
...
ACL_NN_COMMAND(FusedInferAttentionScoreV4);
ACL_NN_COMMAND(FusedInferAttentionScoreV3);
...
ACL_NN_COMMAND(FlashAttentionScoreGradV3);
ACL_NN_COMMAND(FlashAttentionUnpaddingScoreGradV2);
ACL_NN_COMMAND(FlashAttentionUnpaddingScoreGradV3);
ACL_NN_COMMAND(FlashAttentionUnpaddingScoreGradV4);
ACL_NN_COMMAND(FlashAttentionUnpaddingScoreGradV5);
ACL_NN_COMMAND(FlashAttentionScore);
ACL_NN_COMMAND(FlashAttentionVarLenScore);
ACL_NN_COMMAND(FlashAttentionScoreV2);
ACL_NN_COMMAND(FlashAttentionScoreV3);
ACL_NN_COMMAND(FlashAttentionVarLenScoreV2);
ACL_NN_COMMAND(FlashAttentionVarLenScoreV3);
ACL_NN_COMMAND(FlashAttentionVarLenScoreV4);
ACL_NN_COMMAND(FlashAttentionVarLenScoreV5);
ACL_NN_COMMAND(InplaceAttentionWorkerScheduler);
ACL_NN_COMMAND(AttentionUpdate);
ACL_NN_COMMAND(RotaryPositionEmbedding);
ACL_NN_COMMAND(RopeWithSinCosCache);
ACL_NN_COMMAND(NormRopeConcatBackward);
ACL_NN_COMMAND(NormRopeConcat);
ACL_NN_COMMAND(ApplyRotaryPosEmbV2);
ACL_NN_COMMAND(ApplyRotaryPosEmb);
ACL_NN_COMMAND(MoeTokenUnpermuteWithRoutingMap);
ACL_NN_COMMAND(MoeTokenUnpermuteGrad);
ACL_NN_COMMAND(MoeTokenUnpermute);
ACL_NN_COMMAND(MoeTokenPermuteWithRoutingMapGrad);
ACL_NN_COMMAND(MoeTokenPermuteWithRoutingMap);
ACL_NN_COMMAND(MoeTokenPermuteGrad);
ACL_NN_COMMAND(MoeTokenPermute);
ACL_NN_COMMAND(MoeInitRoutingV3);
ACL_NN_COMMAND(MoeInitRoutingV2Grad);
ACL_NN_COMMAND(MoeInitRoutingV2);
ACL_NN_COMMAND(MoeFusedTopk);
ACL_NN_COMMAND(MoeFinalizeRoutingV2Grad);
ACL_NN_COMMAND(MoeFinalizeRoutingV2);
ACL_NN_COMMAND(QuantGroupedMatmulInplaceAdd);
ACL_NN_COMMAND(QuantGroupedMatmulDequant);
ACL_NN_COMMAND(GroupedMatmulSwigluQuantWeightNzV2);
ACL_NN_COMMAND(GroupedMatmulSwigluQuantV2);
ACL_NN_COMMAND(GroupedMatmulSwigluQuantWeightNZ);
ACL_NN_COMMAND(GroupedMatmulSwigluQuant);
ACL_NN_COMMAND(GroupedMatmulFinalizeRoutingWeightNzV2);
ACL_NN_COMMAND(GroupedMatmulFinalizeRoutingWeightNz);
ACL_NN_COMMAND(GroupedMatmulFinalizeRoutingV3);
ACL_NN_COMMAND(GroupedMatmulFinalizeRoutingV2);
ACL_NN_COMMAND(GroupedMatmulFinalizeRouting);
ACL_NN_COMMAND(GroupedMatmulAddV2);
ACL_NN_COMMAND(GroupedMatmulAdd);
ACL_NN_COMMAND(GroupedMatmulWeightNz);
ACL_NN_COMMAND(GroupedMatmulV5);
...
ACL_NN_COMMAND(SparseLightningIndexerGradKLLoss);
ACL_NN_COMMAND(SparseFlashAttentionGrad);
ACL_NN_COMMAND(ScatterPaKvCache);
ACL_NN_COMMAND(NsaCompressGrad);
ACL_NN_COMMAND(MlaProlog);
ACL_NN_COMMAND(MlaPreprocessV2);
ACL_NN_COMMAND(MlaPreprocess);
ACL_NN_COMMAND(GatherPaKvCache);
ACL_NN_COMMAND(RotaryPositionEmbeddingGrad);
ACL_NN_COMMAND(KvRmsNormRopeCache);
ACL_NN_COMMAND(DequantRopeQuantKvcache);
ACL_NN_COMMAND(MoeTokenUnpermuteWithRoutingMapGrad);
ACL_NN_COMMAND(MoeTokenUnpermuteWithEpGrad);
ACL_NN_COMMAND(MoeTokenUnpermuteWithEp);
ACL_NN_COMMAND(MoeTokenPermuteWithEpGrad);
ACL_NN_COMMAND(MoeTokenPermuteWithEp);
...
ACL_NN_COMMAND(MoeGatingTopK);
ACL_NN_COMMAND(MoeFinalizeRouting);
ACL_NN_COMMAND(MoeComputeExpertTokens);
ACL_NN_COMMAND(MoeUpdateExpert);
ACL_NN_COMMAND(MoeDistributeDispatch);
ACL_NN_COMMAND(MoeDistributeCombineAddRmsNorm);
ACL_NN_COMMAND(MoeDistributeCombine);
ACL_NN_COMMAND(MatmulReduceScatterV2);
...
ACL_NN_COMMAND(GroupedMatMulAlltoAllv);
ACL_NN_COMMAND(GroupedMatMulAllReduce);
ACL_NN_COMMAND(DistributeBarrier);
ACL_NN_COMMAND(AlltoAllvGroupedMatMul);
ACL_NN_COMMAND(AllGatherMatmulV2);
ACL_NN_COMMAND(AllGatherMatmul);
ACL_NN_COMMAND(SwinTransformerLnQkvQuant);
ACL_NN_COMMAND(TransQuantParamV3);
ACL_NN_COMMAND(TransQuantParamV2);
// ACL_NN_COMMAND(TransQuantParam); // command without stream
ACL_NN_COMMAND(Quantize);
ACL_NN_COMMAND(GroupQuant);
ACL_NN_COMMAND(FlatQuant);
...
ACL_NN_COMMAND(AdaptiveMaxPool3dBackward);
ACL_NN_COMMAND(AdaptiveMaxPool2dBackward);
ACL_NN_COMMAND(AdaptiveMaxPool3d);
...
ACL_NN_COMMAND(BatchNormReduce);
ACL_NN_COMMAND(BatchNorm);
ACL_NN_COMMAND(FastBatchNormBackward);
ACL_NN_COMMAND(BatchNormBackward);
ACL_NN_COMMAND(BatchNormElemt);
ACL_NN_COMMAND(AddRmsNormQuantV2);
ACL_NN_COMMAND(AddRmsNormQuant);
ACL_NN_COMMAND(AddRmsNormDynamicQuantV2);
ACL_NN_COMMAND(AddRmsNormDynamicQuant);
ACL_NN_COMMAND(AddLayerNormQuant);
ACL_NN_COMMAND(AdaLayerNormV2);
ACL_NN_COMMAND(AdaLayerNormQuant);
ACL_NN_COMMAND(AdaLayerNorm);
ACL_NN_COMMAND(WeightQuantBatchMatmulV3);
ACL_NN_COMMAND(WeightQuantBatchMatmulV2);
ACL_NN_COMMAND(WeightQuantBatchMatmul);
ACL_NN_COMMAND(TransposeBatchMatMul);
ACL_NN_COMMAND(QuantMatmulReduceSumWeightNz);
ACL_NN_COMMAND(QuantMatmulDequant);
ACL_NN_COMMAND(QuantMatmul);
ACL_NN_COMMAND(QuantMatmulV2);
ACL_NN_COMMAND(QuantMatmulV5);
ACL_NN_COMMAND(QuantMatmulWeightNz);
ACL_NN_COMMAND(QuantMatmulV4);
ACL_NN_COMMAND(QuantMatmulV3);
ACL_NN_COMMAND(Mv);
ACL_NN_COMMAND(MatmulCompress);
ACL_NN_COMMAND(Mm);
ACL_NN_COMMAND(Matmul);
ACL_NN_COMMAND(MatmulWeightNz);
ACL_NN_COMMAND(Addmm);
ACL_NN_COMMAND(InplaceAddmm);
ACL_NN_COMMAND(AddmmWeightNz);
ACL_NN_COMMAND(Gemm);
ACL_NN_COMMAND(FusedMatmul);
ACL_NN_COMMAND(FusedLinearOnlineMaxSum);
ACL_NN_COMMAND(FusedLinearCrossEntropyLossGrad);
ACL_NN_COMMAND(ConvertWeightToINT4Pack);
ACL_NN_COMMAND(BatchMatmulQuant);
ACL_NN_COMMAND(Einsum);
ACL_NN_COMMAND(BatchMatMul);
ACL_NN_COMMAND(BatchMatMulWeightNz);
...
ACL_NN_COMMAND(FusedCrossEntropyLossWithMaxSum);
...
ACL_NN_COMMAND(InplaceQuantScatterV2);
...
ACL_NN_COMMAND(ApplyTopKTopP);
ACL_NN_COMMAND(ScaledMaskedSoftmax);
ACL_NN_COMMAND(ScaledMaskedSoftmaxBackward);
ACL_NN_COMMAND(MultiScaleDeformableAttnFunction);
ACL_NN_COMMAND(MultiScaleDeformableAttentionGrad);
ACL_NN_COMMAND(ModulateBackward);
ACL_NN_COMMAND(Modulate);
...
ACL_NN_COMMAND(DeformableConv2d);
ACL_NN_COMMAND(QuantConvolution);
...
ACL_NN_COMMAND(SoftmaxCrossEntropyWithLogits);
...
ACL_NN_COMMAND(AddRelu);
ACL_NN_COMMAND(InplaceAddRelu);
...
ACL_NN_COMMAND(BinaryCrossEntropyWithLogitsTargetBackward);
...
ACL_NN_COMMAND(Heaviside);
ACL_NN_COMMAND(HardtanhBackward);
ACL_NN_COMMAND(HardswishBackwardV2);
...
ACL_NN_COMMAND(FastGeluBackward);
ACL_NN_COMMAND(FastGelu);
...
ACL_NN_COMMAND(SilentCheckV2);
ACL_NN_COMMAND(SilentCheck);
ACL_NN_COMMAND(ObfuscationSetupV2);
ACL_NN_COMMAND(ObfuscationCalculateV2);
ACL_NN_COMMAND(BidirectionLSTMV2);
ACL_NN_COMMAND(BidirectionLSTM);
ACL_NN_COMMAND(SwiGluQuantV2);
ACL_NN_COMMAND(SwiGluQuant);
ACL_NN_COMMAND(DynamicBlockQuant);
ACL_NN_COMMAND(DequantSwigluQuantV2);
ACL_NN_COMMAND(DequantSwigluQuant);
ACL_NN_COMMAND(DequantBias);
ACL_NN_COMMAND(ApplyFusedEmaAdam);
ACL_NN_COMMAND(ApplyAdamWQuant);
ACL_NN_COMMAND(AdvanceStepV2);
ACL_NN_COMMAND(AdvanceStep);
ACL_NN_COMMAND(RmsNormGrad);
ACL_NN_COMMAND(RmsNorm);
ACL_NN_COMMAND(MaskedSoftmaxWithRelPosBias);
ACL_NN_COMMAND(InplaceAddRmsNorm);
ACL_NN_COMMAND(GroupNormSwishGrad);
ACL_NN_COMMAND(GroupNormSwish);
ACL_NN_COMMAND(GemmaRmsNorm);
ACL_NN_COMMAND(DeepNormGrad);
ACL_NN_COMMAND(DeepNorm);
ACL_NN_COMMAND(AddRmsNormCast);
ACL_NN_COMMAND(AddRmsNorm);
ACL_NN_COMMAND(AddLayerNormGrad);
ACL_NN_COMMAND(AddLayerNorm);
ACL_NN_COMMAND(LogitGrad);
ACL_NN_COMMAND(Logit);
ACL_NN_COMMAND(CrossEntropyLossGrad);
ACL_NN_COMMAND(CrossEntropyLoss);
ACL_NN_COMMAND(TopKTopPSample);
ACL_NN_COMMAND(RepeatInterleaveGrad);
ACL_NN_COMMAND(ExpandIntoJaggedPermute);
...
ACL_NN_COMMAND(ForeachNonFiniteCheckAndUnscale);
...
ACL_NN_COMMAND(SquaredRelu);
ACL_NN_COMMAND(GeluMul);
ACL_NN_COMMAND(FatreluMul);
ACL_NN_COMMAND(ClippedSwiglu);
ACL_NN_COMMAND(MoeDistributeCombineV3);
...
ACL_NN_COMMAND(MoeDistributeDispatchV3);
...
ACL_NN_COMMAND(DistributeBarrierV2);
ACL_NN_COMMAND(MoeDistributeDispatchV2);
ACL_NN_COMMAND(MoeDistributeCombineV2);
ACL_NN_COMMAND(QuantMatmulAllReduceV2);
ACL_NN_COMMAND(MoeDistributeDispatchV4);
ACL_NN_COMMAND(AlltoAllAllGatherBatchMatMul);
ACL_NN_COMMAND(WeightQuantMatmulAllReduce);
ACL_NN_COMMAND(MatmulAllReduceV2);
ACL_NN_COMMAND(MoeDistributeCombineV4);
